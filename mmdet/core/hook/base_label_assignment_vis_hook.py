...
class BaseLabelAssignmentVisHook(Hook):
    ...
    def __init__(self,
                 ...
                 num_images=None,
                 rate='epoch'):
        self.sample_idxs = [sample_idxs] if isinstance(sample_idxs,
                                                       int) else sample_idxs
        if num_images is not None and isinstance(sample_idxs, int):
            self.sample_idxs = [_ for _ in range(num_images)]
            warnings.warn(f"parameter 'sample_idxs' must be a list "
                          f"when 'num_images' is not None, "
        ...
        # we set this explicit in before_train_epoch instead of using
        # before_run hook because
        # before_run doesn't contain dataset or dataloader
        self.sampled = False

    def before_run(self, runner):
        self.out_dir = runner.work_dir
        self.file_client = FileClient.infer_client(None, self.out_dir)
        self.out_dir = self.file_client.join_path(self.out_dir,
                                                  'LabelAssignmentVis')
        runner.logger.info(f'Label Assignment Visualization will be saved to '
                           f'{self.out_dir} by '
    ...
    def after_train_iter(self, runner):
        if self.rate == 'iter':
            ...
            self._plot_results(assign_matrices, strides, priors_per_level,
                               featmap_sizes, runner)
    ...
    def _get_assign_results(self, runner):
        """This will execute label assignment from the start for only the
        images in self.image_list.

        Since every model has its own implementation of label assignment,
        every specific label assignment start will inherit from this Base
        and execute its own label assignment
        ...
        4. Perform label assignment to get
        `assign_result` and `sampling_result`
        5. Return `assign_matrices`, `strides` and `multi_priors_per_level`
        """
        pass

    def _plot_results(self, assign_matrices, strides, multi_priors_per_level,
                      multi_featmap_sizes, runner):
        for (image, image_metas,
             gt_bboxes, gt_label,
             assign_matrix, stride,
             priors_per_level, featmap_sizes) in \
                zip(
                    self.image_list,
                    self.img_metas_list,
                    self.gt_bboxes_list,
            ...
            assert len(stride) == len(
                priors_per_level
            ), 'Number of level must equal to number of strides'
            results = []
            ...
            for i in range(len(priors_per_level)):
                ...
                num_priors_from_prev_levels += num_priors_level_i
                matrix_level_i = matrix_level_i.view(
                    (featmap_size_level_i[0], featmap_size_level_i[1]))
                # this will return a list of 2D position of
                # where the label is non-zero on the matrix
            ...
            for i, gt_bbox in enumerate(gt_bboxes):
                ...
                np_image = cv2.rectangle(np_image.copy(), gt_bbox[:2],
                                         gt_bbox[2:], colors(gt_id))
            # cv2.imwrite(
            # osp.join(self.out_dir, image_name + str(counter) + ".jpg"),
            # np_image)
