...
class BaseShowDataPipline(Hook):
    ...
    def before_run(self, runner):
        runner.logger.info('BaseShowDataPipline will be running!!')
        if save:
            ...
            self.out_dir = self.file_client.join_path(self.out_dir,
                                                      'DataPipline')
            runner.logger.info(
                f'img  Visualization will be saved to {self.out_dir} by '
    ...
    def before_train_epoch(self, runner):
        ...
        for i, data_batch in enumerate(train_loader):
            ...
            image_name = osp.splitext(
                data_batch['img_metas']._data[0][1]['ori_filename'])[0]
            for batch_i in range(len(img_batch)):
                ...
                img_numpy_float = mmcv.imdenormalize(img_hwc, mean_value,
                                                     std_value)
