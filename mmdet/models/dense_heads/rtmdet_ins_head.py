...
class RTMDetInsHead(RTMDetHead):
    ...
    def loss_by_feat(self,
        ...
        losses_cls, losses_bbox, cls_avg_factors, bbox_avg_factors = multi_apply(  # noqa E501
            ...
            label_weights_list,
            bbox_targets_list,
            assign_metrics_list,  # noqa E501
            self.prior_generator.strides)  # noqa E501
...
@MODELS.register_module()
class RTMDetInsSepBNRegHead(RTMDetInsSepBNHead):
    ...
    def __init__(self,
        ...
        super().__init__(
            ...
            with_objectness=with_objectness,
            **kwargs)
        self.loss_tail = nn.MSELoss(reduction='mean')
    ...
    def init_weights(self) -> None:
        ...
        for rtm_tall in self.rtm_tall:
            normal_init(rtm_tall.conv, std=0.01, bias=bias_cls)
    ...
    def forward(self, feats: Tuple[Tensor, ...]) -> tuple:
        ...
        for idx, (x, stride) in enumerate(
            ...
            tall = self.rtm_tall[idx](reg_feat)
            tall_preds.append(tall)
            ...
            kernel_preds.append(kernel_pred)
        return tuple(cls_scores), tuple(bbox_preds), tuple(
            kernel_preds), tuple(tall_preds), mask_feat

    def predict_by_feat(self,
                        cls_scores: List[Tensor],
                        bbox_preds: List[Tensor],
                        kernel_preds: List[Tensor],
                        tall_preds: List[Tensor],
                        mask_feat: Tensor,
                        score_factors: Optional[List[Tensor]] = None,
                        batch_img_metas: Optional[List[dict]] = None,
                        cfg: Optional[ConfigType] = None,
        ...
        for img_id in range(len(batch_img_metas)):
            ...
            kernel_pred_list = select_single_mlvl(
                kernel_preds, img_id, detach=True)
            tail_pred_list = select_single_mlvl(
                tall_preds, img_id, detach=True)
            if with_score_factors:
                score_factor_list = select_single_mlvl(
                    score_factors, img_id, detach=True)
            else:
    ...
    def _predict_by_feat_single(self,
        ...
        for level_idx, (cls_score, bbox_pred, kernel_pred,
                        tail_pred, score_factor, priors) in \
                enumerate(zip(cls_score_list, bbox_pred_list, kernel_pred_list,
                              tail_pred_list, score_factor_list, mlvl_priors)):
            ...
            kernel_pred = kernel_pred.permute(1, 2, 0).reshape(
                -1, self.num_gen_params)
            tail_pred = tail_pred.permute(1, 2, 0).reshape(-1, 1)
            if self.use_sigmoid_cls:
                scores = cls_score.sigmoid()
            else:
                # remind that we set FG labels to [0, num_class-1]
            ...
            priors = filtered_results['priors']
            kernel_pred = filtered_results['kernel_pred']
            tail_pred = filtered_results['tail_pred']
        ...
        results.labels = torch.cat(mlvl_labels)
        results.kernels = torch.cat(mlvl_kernels)
        results.talls = torch.cat(mlvl_talls)
    ...
    def loss_by_feat_single(self, cls_score: Tensor, bbox_pred: Tensor,
                            tall_pred: Tensor, labels: Tensor,
                            label_weights: Tensor, bbox_targets: Tensor,
                            tall_targets: Tensor, assign_metrics: Tensor,
                            stride: List[int]):
        assert stride[0] == stride[1], 'h stride is not equal to w stride!'
        cls_score = cls_score.permute(0, 2, 3, 1).reshape(
            -1, self.cls_out_channels).contiguous()
        tall_pred = tall_pred.permute(0, 2, 3, 1).reshape(-1, 1).contiguous()
        tall_targets = tall_targets.reshape(-1, 1)
        ...
        loss_tall = self.loss_tail(tall_pred, tall_targets)
        ...
        return loss_cls, loss_bbox, loss_tall, assign_metrics.sum(
        ), pos_bbox_weight.sum()
    ...
    def loss_by_feat(self,
        ...
        flatten_tall_preds = torch.cat([
            tall_pred.permute(0, 2, 3, 1).reshape(num_imgs, -1, 1)
            for tall_pred in tall_preds
        ], 1)
        ...
        cls_reg_targets = self.get_targets(
            ...
            batch_gt_instances_ignore=batch_gt_instances_ignore)
        (anchor_list, labels_list, label_weights_list, bbox_targets_list,
         talls_list, assign_metrics_list,
         sampling_results_list) = cls_reg_targets

        losses_cls, losses_bbox, losses_tall, cls_avg_factors, bbox_avg_factors = multi_apply(  # noqa E501
            self.loss_by_feat_single,
            cls_scores,
            decoded_bboxes,
            tall_preds,  # noqa E501
            labels_list,
            label_weights_list,
            bbox_targets_list,
            talls_list,  # noqa E501
            assign_metrics_list,
            self.prior_generator.strides)  # noqa E501

        cls_avg_factor = reduce_mean(sum(cls_avg_factors)).clamp_(min=1).item()
        losses_cls = list(map(lambda x: x / cls_avg_factor, losses_cls))
        losses_tall = list(map(lambda x: x / cls_avg_factor, losses_tall))
    ...
    def get_targets(self,
        ...
        (all_anchors, all_labels, all_label_weights, all_bbox_targets,
         all_talls, all_assign_metrics, sampling_results_list) = multi_apply(
        ...
        assign_metrics_list = images_to_levels(all_assign_metrics,
                                               num_level_anchors)
        talls_list = images_to_levels(all_talls, num_level_anchors)

        return (anchors_list, labels_list, label_weights_list,
                bbox_targets_list, talls_list, assign_metrics_list,
                sampling_results_list)
    ...
    def _get_targets_single(self,
        ...
        talls = anchors.new_full((num_valid_anchors, ), 0, dtype=torch.float)
        gt_talls = torch.from_numpy(img_meta['gt_bboxes_talls']).to(anchors)
        ...
        if len(pos_inds) > 0:
            ...
            talls[pos_inds] = gt_talls[sampling_result.pos_assigned_gt_inds]
        ...
        if unmap_outputs:
            ...
            assign_metrics = unmap(assign_metrics, num_total_anchors,
                                   inside_flags)
            talls = unmap(talls, num_total_anchors, inside_flags)
        return (anchors, labels, label_weights, bbox_targets, talls,
